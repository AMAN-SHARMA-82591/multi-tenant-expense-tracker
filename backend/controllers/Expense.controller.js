...
export const getExpenseList = asyncHandler(async (req, res) => {
  const { tenantId = null } = req.params;
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 10;
  const skip = (page - 1) * limit;
  const [result] = await ExpenseModel.aggregate([
    { $match: { createdBy: ObjectId(req.uid), tenantId: ObjectId(tenantId) } },
    {
      $facet: {
...
export const generateSummaryReport = asyncHandler(async (req, res) => {
...
export const createExpense = asyncHandler(async (req, res) => {
  ...
  const { title, category, amount, date, tenantId = null } = data;
